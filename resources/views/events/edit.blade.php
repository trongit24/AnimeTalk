...
@section('title', 'Edit Event - AnimeTalk')
            ...
            <form action="{{ route('events.update', $event->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; margin-bottom: 1.5rem;">
                    ...
                    <div>
                        ...
                        <input type="datetime-local" name="end_time" value="{{ old('end_time', $event->end_time?->format('Y-m-d\TH:i')) }}"
                               style="width: 100%; padding: 0.75rem 1rem; border: 1px solid #e0e0e0; border-radius: 8px; font-size: 1rem;">
                        @error('end_time')
                        <span style="color: red; font-size: 0.875rem; margin-top: 0.25rem; display: block;">{{ $message }}</span>
                        @enderror
                ...
                <div style="margin-bottom: 1.5rem;">
                    <label style="display: block; font-weight: 600; margin-bottom: 0.5rem; color: #333;">
                        {{ $event->cover_image ? 'Change Cover Image' : 'Cover Image' }}
                    </label>
                ...
                <div style="margin-bottom: 2rem;">
                    ...
                    <div style="display: flex; flex-direction: column; gap: 0.75rem;">
                        <label style="display: flex; align-items: center; gap: 0.75rem; padding: 0.75rem; border: 1px solid #e0e0e0; border-radius: 8px; cursor: pointer;">
                            <input type="radio" name="privacy" value="public" {{ old('privacy', $event->privacy) == 'public' ? 'checked' : '' }} required
                        ...
                        <label style="display: flex; align-items: center; gap: 0.75rem; padding: 0.75rem; border: 1px solid #e0e0e0; border-radius: 8px; cursor: pointer;">
                            <input type="radio" name="privacy" value="friends" {{ old('privacy', $event->privacy) == 'friends' ? 'checked' : '' }}
                                   style="width: 18px; height: 18px;">
                            <div>
                        ...
                        <label style="display: flex; align-items: center; gap: 0.75rem; padding: 0.75rem; border: 1px solid #e0e0e0; border-radius: 8px; cursor: pointer;">
                            <input type="radio" name="privacy" value="private" {{ old('privacy', $event->privacy) == 'private' ? 'checked' : '' }}
                                   style="width: 18px; height: 18px;">
                            <div>
